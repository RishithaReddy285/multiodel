...
async function analyzeDocument(file) {
  ...
  try {
    if (file.mimetype === 'application/pdf') {
      ...
      const safeText = data.text.length > 25000 ? data.text.substring(0, 25000) + "\n...[Content Truncated due to API Limits]..." : data.text;
      ...
      const safeText = text.length > 25000 ? text.substring(0, 25000) + "\n...[Content Truncated due to API Limits]..." : text;
      ...
      content: `Always respond strictly in the following JSON format without markdown blocks or extra text:
{
  ...
  "suggestions": ["List of optimization and improvement suggestions"]
    ...
    return {
      ...
      explanation: `Error details: ${error.stack || error.message || error}`,
      issues: [],
      suggestions: []
    };
  }
